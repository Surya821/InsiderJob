...
const RecruiterLogin = () => {
    ...
    return (
        <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
            <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-5 00'>
                <h1 className='text-center text-2xl text-neutral-700 font-medium'>Recruiter {state}</h1>
                <p className='text-sm text-neutral-600'>Welcome Back! Please sign in to continue</p>
                ...
                : <>
                    ...
                    <div className='border border-gray-200 px-4 py-2 flex items-center gap-2 rounded-full mt-5 '>
                        ...
                        <input className='outline-none text-sm' onChange={e=> setEmail(e.target.value)} type="text" placeholder='Email Id' required />
                    </div>
                ...
                <button type='submit' className='bg-blue-600 w-full text-white py-2 rounded-full cursor-pointer mt-4'>
                    {state === 'Login' ? 'Login' : isTextDataSubmitted ? 'Create Account': 'Create Account'}
                </button>
